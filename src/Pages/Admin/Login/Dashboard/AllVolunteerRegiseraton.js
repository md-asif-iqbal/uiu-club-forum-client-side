...
const AllVolunteerRegiseraton = () => {
     const url = `http://localhost:8000/upcomingEvents`;
     const {
       data: upcomingEvents,
       isLoading,
       refetch,
            ...
            <nav className="flex items-center  px-5 rounded-full lg:text-lg  font-bold justify-between max-w-2xl p-4 mx-auto">
              <ul className="flex mx-auto  space-x-5  text-xs lg:text-lg font-bold ">
                {upcomingEvents.map((events) => (
                  <li className="px-1 lg:px-2 py-2 hover:bg-primary hover:text-white  rounded-lg text-secondary">
                    <CustomLink to={`/Dashboard/volunteerRequest/${events.title}`}>
                      {events.title}
                    </CustomLink>
                  </li>
                ))}
